...
function changeAll() {
    ...
    document.getElementById('changetxthex').innerHTML = pic_color("rgb(" + r + "," + g + "," + b + ")").toHexString();
}

function hslLum_top() {
    var i, a, match;
    var color = document.getElementById("colorhexDIV").innerHTML;
    var hslObj = pic_color(color);
    var h = hslObj.hue;
    var s = hslObj.sat;
    var l = hslObj.lightness;
    var arr = [];
    for (i = 0; i <= 20; i++) {
        arr.push(pic_color("hsl(" + h + "," + s + "," + (i * 0.05) + ")"));
    }
    arr.reverse();
    a = "<h3 class='pic-center'>Lighter / Darker:</h3><table class='colorTable' style='width:100%;'>";
    match = 0;
    for (i = 0; i < arr.length; i++) {
        if (match == 0 && Math.round(l * 100) == Math.round(arr[i].lightness * 100)) {
            ...
            a += "<td style='background-color:" + pic_color(hslObj).toHexString() + "'><br><br></td>";
            a += "<td>&nbsp;<b>" + pic_color(hslObj).toHexString() + "</b></td>";
            a += "</tr>";
            a += "<tr><td></td><td></td><td></td></tr>";
            match = 1;
        } else {
            if (match == 0 && l > arr[i].lightness) {
                ...
                a += "<tr>";
                a += "<td style='text-align:right;'><b>" + Math.round(l * 100) + "%&nbsp;</b></td>";
                a += "<td style='background-color:" + pic_color(hslObj).toHexString() + "'></td>";
                a += "<td>&nbsp;<b>" + pic_color(hslObj).toHexString() + "</b></td>";
                a += "</tr>";
                a += "<tr><td></td><td></td><td></td></tr>";
                match = 1;
...
function hslTable(x) {
    ...
    for (i = 0; i <= lineno; i++) {
        if (x == "hue") { arr.push(pic_color("hsl(" + (i * 15) + "," + s + "," + l + ")")); }
        if (x == "sat") { arr.push(pic_color("hsl(" + h + "," + (i * 0.05) + "," + l + ")")); }
        if (x == "light") { arr.push(pic_color("hsl(" + h + "," + s + "," + (i * 0.05) + ")")); }
    }
    if (x == "sat" || x == "light") { arr.reverse(); }
    a = "<h3>" + header + "</h3>";
    a += "<div class='table-responsive'>";
    a += "<table class='pic-table-all colorTable' style='width:100%;white-space: nowrap;font-size:14px;'>";
    a += "<tr>";
    ...
    for (i = 0; i < arr.length; i++) {
        same = 0;
        if (x == "hue") {
            loopHSL = pic_color(arr[i]).hue;
            HSL = h;
            if (i == arr.length - 1) { loopHSL = 360; }
            comp = (loopHSL > HSL);
        }
        if (x == "sat") {
            loopHSL = Math.round(pic_color(arr[i]).sat * 100);
            HSL = Number(s * 100);
            comp = (loopHSL < HSL);
            HSL = HSL + "%";
            loopHSL = loopHSL + "%";
        ...
        if (x == "light") {
            loopHSL = Math.round(pic_color(arr[i]).lightness * 100);
            HSL = Number(l * 100);
            comp = (loopHSL < HSL);
            HSL = HSL + "%";
            loopHSL = loopHSL + "%";
